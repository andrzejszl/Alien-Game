<template>
  <GamestateStart v-if="uiState === 'start'">
    ...
    <p v-for="option in characterChoices" :key="option" class="character-choices">
      <input v-model="characterInput" type="radio" :value="option" :id="option">
  ...
  <GamestateFinish v-else :uiState="uiState"></GamestateFinish>
</template>
...
export default {
  components: { Zombie, Score, Friend, Artist, Baker, Mechanic, GamestateStart, GamestateFinish },
  data() {
    return {
      characterInput: '',
    }
